  ...
  try {
    ...
    for (const ingredient of ingredients) {
      if (typeof ingredient !== "string") {
        return new Response(JSON.stringify({ error: "All ingredients must be strings" }), {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        });
      }
      ...
      try {
        ...
        if (searchResponse.ok) {
          ...
          if (compound) {
            ...
            const molecularWeight = compound.props?.find((p: any) => p.urn?.label === "Molecular Weight")?.value?.fval;
