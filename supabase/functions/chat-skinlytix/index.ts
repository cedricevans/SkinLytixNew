...
serve(async (req) => {
  ...
  try {
    ...
    const { data: analysis, error } = await supabase.from("user_analyses").select("*").eq("id", analysisId).single();
    ...
    const recommendations =
      typeof analysis.recommendations_json === "string"
...
- If you mention professional referral, start response with "⚕️ REFERRAL:" so it can be detected`;
    ...
    const callGeminiDirect = async (): Promise<Response> => {
      ...
      console.warn("Falling back to Gemini direct API");
