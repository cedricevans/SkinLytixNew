...
import { lookupIngredientKnowledge } from "../_shared/ingredient-knowledge.ts";
...
Keep it conversational, specific, and non-technical. No medical claims. Avoid repeating the same point.`;
    ...
    if (!geminiApiKey) return "No detailed information available.";
    ...
    return text.trim() || "No detailed information available.";
  } catch (error) {
...
${JSON.stringify(formattedAnalysis, null, 2)}`;
    ...
    const data = await response.json();
    const gptContent = data?.candidates?.[0]?.content?.parts?.[0]?.text ?? "";
...
function generatePersonalizedSummary(
  ...
  const skinConcerns =
    Array.isArray(profile.skin_concerns) && profile.skin_concerns.length > 0 ? profile.skin_concerns : [];
  let summary = "";
  ...
  if (score >= 70) {
    ...
    if (beneficialIngredients.length > 0) {
      const topBeneficial = beneficialIngredients
        .slice(0, 2)
        .map((i) => i.name)
        .join(" and ");
...
serve(async (req) => {
  ...
  try {
    ...
    if (!product_name || !ingredients_list || !user_id) {
      return new Response(JSON.stringify({ error: "product_name, ingredients_list, and user_id are required" }), {
        status: 400,
    ...
    const getProductType = (category: string): "face" | "body" | "hair" | "other" => {
      ...
      // FACE
      if (
        ["face-cleanser", "serum", "face-moisturizer", "sunscreen", "toner", "mask", "eye-cream"].includes(
          lowerCategory,
        )
      ) {
        return "face";
    ...
    if (barcode) {
      ...
      if (cachedProduct) {
        ...
        const obfResponse = await fetch(`${supabaseUrl}/functions/v1/query-open-beauty-facts`, {
          method: "POST",
          headers: {
    ...
    console.log(
      `Product categorization: "${extractedCategory}" (${extractedBrand}) → productType: "${productType}" | Source: ${detectionSource}`,
    );
    ...
    const ingredientKnowledge: Record<string, any> = {
      ...
      retinol: {
        ...
        tips: [
          "Start 2-3x/week and build tolerance",
      ...
      "hyaluronic acid": {
        ...
        tips: [
          ...
          "Use in humid environments or mist face first",
        ],
        product_types: [],
      ...
      peptide: {
        ...
        tips: [
          ...
          "Consistent use shows results in 8-12 weeks",
        ],
        product_types: ["face"],
        sunSensitivity: false,
        category: "active",
      ...
      urea: {
        ...
        conflicts: [],
        tips: [
          "Especially effective at 10%+ concentration",
          "Apply to rough patches (elbows, knees, feet)",
      ...
      aluminum: {
        ...
        tips: [
          "Apply to completely dry skin",
          "Most effective antiperspirant ingredient",
          "Some prefer aluminum-free alternatives",
        ],
        product_types: ["body"],
        sunSensitivity: false,
        category: "active",
      ...
      "zinc pyrithione": {
        ...
        tips: [
          ...
          "Use 2-3x/week for active dandruff",
          "Safe for daily use once controlled",
        ],
        product_types: ["hair"],
        sunSensitivity: false,
        category: "active",
    ...
    const getTimingRecommendations = (actives: Array<{ name: string; info: any }>, prodType: string): string[] => {
      ...
      if (amPreferred.length > 0) {
        suggestions.push(
          `☀️ Best used in AM - ${amPreferred.map((a) => a.name).join(", ")} works well in morning routine`,
        );
      }
    ...
    const getConcernGuidance = (
      concerns: string[],
      actives: Array<{ name: string; info: any }>,
      prodType: string,
      profile: any,
    ): string[] => {
      const suggestions = [];
      ...
      if (
        prodType === "body" &&
        concerns.some((c) => c.includes("body-") || c.includes("eczema") || c.includes("keratosis"))
      ) {
    ...
    const generateSummary = (score: number, profile: any, prodType: string): string => {
      ...
      if (prodType === "face") {
        ...
        const bodyConcerns = profile?.body_concerns || [];
        context =
          bodyConcerns.length > 0 ? `your body (addressing ${bodyConcerns.slice(0, 2).join(", ")})` : "your body";
      } else if (prodType === "hair") {
    ...
    const getGeneralProductGuidance = (
      category: string,
      prodType: string,
      profile: any,
      productName: string,
      detectedActives: Array<{ name: string; info: any }>,
    ): string[] => {
      const suggestions = [];
      ...
      if (prodType === "body") {
        if (lowerCategory.includes("oil")) {
          ...
          suggestions.push(
            `🌙 ${productName} works best in evening routine—can be mixed with lotion for lighter texture`,
          );
        ...
        if (lowerCategory.includes("shampoo") || lowerCategory.includes("beard wash")) {
          ...
          suggestions.push(
            `💧 Apply ${productName} to damp (not wet) hair for natural finish, dry hair for maximum hold`,
          );
        ...
        if (lowerCategory.includes("cleanser")) {
          suggestions.push(
            `🌡️ Use ${productName} with lukewarm water—hot water strips oils, cold water doesn't cleanse well`,
          );
          ...
          suggestions.push(
            `✨ Apply ${productName} after cleansing to target ${profile?.skin_concerns?.[0]?.replace(/-/g, " ") || "your concerns"}`,
          );
          ...
          const concernText = profile?.skin_concerns?.[0]
            ? ` to address ${profile.skin_concerns[0].replace(/-/g, " ")}`
            : "";
          suggestions.push(
            `🌙 Use ${productName} in your ${activesText.includes("aha") || activesText.includes("bha") ? "PM" : "AM & PM"} routine after cleansing${concernText}`,
          );
          if (lowerCategory.includes("pad")) {
            ...
            if (activesText.includes("aha") || activesText.includes("bha") || activesText.includes("acid")) {
              suggestions.push(
                "⏱️ Use 2-3 times per week initially—exfoliating acids can increase sensitivity with daily use",
              );
            }
          } else {
    ...
    for (const active of detectedActives.slice(0, 2)) {
      if (active.info.tips && routineSuggestions.length < 8) {
        ...
        if (productTypes.length > 0 && !productTypes.includes(productType)) {
          console.log(
            `Skipping ${active.name} tips - not relevant for ${productType} products (valid for: ${productTypes.join(", ")})`,
          );
          continue;
        }
    ...
    const shouldUseAiIngredientExplanations =
      Boolean(geminiApiKey) && !skip_ingredient_ai_explanations && !isQuickScan;
    ...
    const getFallbackIngredientExplanation = (name: string, role: string): string => {
      ...
      const specific: Record<string, string> = {
        "sodium levulinate":
          "A plant-derived preservative helper that supports formula freshness and can feel gentle on skin.",
    ...
    const enrichedSafeIngredients = await Promise.all(
      safe.map(async (ingredientName: string, index: number) => {
        const pubchemData = ingredientResults.find(
          (r: any) =>
        ...
        if (shouldUseAiIngredientExplanations && index < maxAiSafeExplanations) {
          explanation = await generateIngredientExplanation(ingredientName, "safe", pubchemData, geminiApiKey);
        }
    ...
    const enrichedConcernIngredients = await Promise.all(
      concerns.map(async (ingredientName: string, index: number) => {
        ...
        if (shouldUseAiIngredientExplanations && index < maxAiConcernExplanations) {
          explanation = await generateIngredientExplanation(ingredientName, "unverified", pubchemData, geminiApiKey);
        }
    ...
    const calculateSubScores = () => {
      ...
      const skinCompatibility = profile ? Math.min(100, 50 + beneficialCount * 8 - problematicCount * 5) : 50;
    ...
    if (geminiApiKey && !skip_ai_explanation && !isQuickScan) {
      ...
      console.warn("GEMINI_API_KEY not configured - skipping GPT explanation");
    }
