...
console.log('╔═══════════════════════════════════════════════════════════════════════════════════╗');
console.log('║                   🔄 RESTORING DATA FROM BACKUP CSVs                            ║');
console.log('╚═══════════════════════════════════════════════════════════════════════════════════╝\n');
...
const analyses = parseCSV(analysesCSV);
...
for (let i = 0; i < analyses.length; i += batchSize) {
  ...
  if (error) {
    ...
    console.log(`✅ Batch ${i / batchSize + 1}: ${batch.length} records inserted (${inserted}/${analyses.length})`);
  }
}
...
for (let i = 0; i < events.length; i += batchSize) {
  ...
  if (error) {
    ...
    inserted += batch.length;
    console.log(`✅ Batch ${i / batchSize + 1}: ${batch.length} records inserted (${inserted}/${events.length})`);
  }
}
